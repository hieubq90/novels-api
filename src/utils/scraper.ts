...
class NovelScraper {
  ...
  private async getNovels(path: string, page = 1): Promise<IGetNovels> {
    try {
      ...
      const novels = Array.from($('.col-truyen-main .list-truyen .row')).map((item) => {
        ...
        const is_new = label_title ? label_title.includes('label-new') : false;
        const is_full = label_title ? label_title.includes('label-full') : false;
  ...
  public async getNovelsOfAuthor(
    aid: string
  ): Promise<IGetNovels> {
    try {
  ...
  public async getNovelDetail(nid: string): Promise<Novel> {
    try {
      ...
      const authorA = this.trim($('.info-holder a[itemprop=author]').attr('href'));
      const author = {
        id: authorA ? authorA.split('/').at(-1) : '',
        name: this.trim($('.info-holder a[itemprop=author]').text()) || 'Đang cập nhật'
      }
      const genres = Array.from($('.info-holder a[itemprop=genre]')).map((item) => ({
        id: this.trim($(item).attr('href')).split('/').at(-1),
        name: this.trim($(item).attr('title')),
      }))
      const description = Array.from($(".desc section.limit-desc p")).slice(0, -1).map((item) => `<p>${$(item).prop('innerHTML')}</p>`).join();
      const lastest_chapters = Array.from($('ul.l-chapters li a')).map((item) => ({
        id: this.trim($(item).attr('href')).split('/').at(-1),
        name: this.trim($(item).attr('title')),
      }))
  ...
  public async getChapterContent(nid: string, cid: string): Promise<IChapterContent> {
    try {
      const $ = await this.createRequest(`${nid}/${cid}`)
      const content = $('#chapter-content')
      $('div', content).remove()
      return { content: this.trim($(content).prop('innerHTML')).replace('\n', '') }
    } catch (err) {
      throw err;
    }
  }
